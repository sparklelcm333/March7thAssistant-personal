...
class HomeInterface(ScrollArea):
    ...
    def _build_card_action(self, card_data):
        ...
        if card_data.get("action_type") == "single":
            ...
            return lambda tid=task_id: self._start_task_with_component_check(tid)
        else:
            result = {}
            for item in card_data.get("menu_items", []):
                ...
                if task_id in HOME_EXTRA_TASKS:
                    ...
                    result[label] = lambda tid=task_id: self._start_task_with_component_check(tid)
            return result

    def _start_task_with_component_check(self, task_id: str):
        """启动任务前预检组件（universe/fight 缺失时先引导去组件管理）。"""
        from module.update.component_manager import ensure_component_before_task
        if not ensure_component_before_task(task_id, self.window()):
            return
        from tasks.base.tasks import start_task
        start_task(task_id)

    def _open_component_dir(self, task_id: str, path: str):
        """打开组件目录：组件缺失时引导去组件管理，就绪才打开目录。"""
        from module.update.component_manager import ensure_component_before_task
        if not ensure_component_before_task(task_id, self.window()):
            return
        os.startfile(path)
    ...
    def _get_extra_task_action(self, task_id):
        """获取特殊主页操作的 lambda"""
        if task_id == "_reset_universe_config":
            ...
            return lambda: self._open_component_dir("universe", cfg.universe_path)
            ...
            return lambda: self._open_component_dir("fight", cfg.fight_path)
