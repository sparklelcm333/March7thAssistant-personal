...
from qfluentwidgets import (MessageBox, LineEdit, ComboBox, EditableComboBox, DateTimeEdit,
                            ...
                            isDarkTheme, InfoBar, InfoBarPosition, PushButton, SpinBox, CheckBox, SimpleCardWidget,
                            TextBrowser, setCustomStyleSheet)
from typing import Optional
from app.common.scroll_dialog import SplitFadeDialogMixin
...
class MessageBoxHtmlUpdate(SplitFadeDialogMixin, MessageBox):
    def __init__(self, title: str, content: str, parent=None):
        ...
        self.textLayout.removeWidget(self.contentLabel)
        self.contentLabel.clear()
        self._scroll = None  # TextBrowser 自带滚动，无需外置 QScrollArea

        # 更新日志用 TextBrowser：HTML 渲染 + 自动换行 + 滚动条
        # （BodyLabel 是 QLabel，长内容会截断不滚动）
        self.contentLabel = TextBrowser(parent)
        self.contentLabel.setObjectName("contentLabel")
        self.contentLabel.setHtml(content)
        self.contentLabel.setOpenExternalLinks(True)
        self.contentLabel.anchorClicked.connect(self.open_url)
        self.contentLabel.setMinimumWidth(500)
        self.contentLabel.setMinimumHeight(300)
...
class MessageBoxUpdate(MessageBoxHtmlUpdate):
    """发现新版本弹窗：yesButton（立即更新）触发更新流程，cancelButton（暂不更新）关闭弹窗。"""
    ...
    def __init__(self, title: str, content: str, parent=None):
        ...
        self.yesButton.setText(tr('立即更新'))
        self.yesButton.clicked.connect(lambda: self.updateRequested.emit())
        self.cancelButton.setText(tr('暂不更新'))
