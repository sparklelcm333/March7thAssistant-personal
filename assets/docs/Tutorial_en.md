...
If you need to work with Windows Task Scheduler, or you want to run the full workflow directly, you can also use `March7th Launcher.exe daily`.

### Launch from Command Line

`March7th Launcher.exe` supports command-line arguments; specifying a task name runs it in headless mode (without launching the GUI).
...
`March7th Launcher.exe daily` (headless mode, run Daily Training without launching the GUI)
...
If you know you want to run a task but forgot the exact task name, run `March7th Launcher.exe --help` to see the latest list.
...
When a new version pop-up appears, click download. You can also click `Check for updates` at the bottom of the settings page.

Updates are applied automatically via incremental patches: the main program downloads the patch, applies it, and restarts with the new version.
