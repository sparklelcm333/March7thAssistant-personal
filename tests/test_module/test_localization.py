...
class TestTr:
    ...
    def test_translation_not_found_zh_cn(self):
        ...
        try:
            # 非空翻译（模拟已加载），key 缺失 → zh_CN 返回原文
            loc._translations = {"其他key": "其他值"}
    ...
    def test_translation_not_found_en_us(self):
        ...
        original_lang = loc._current_lang
        try:
            # 非空翻译（模拟已加载），key 缺失 → en_US 返回 Missing 占位
            loc._translations = {"其他key": "other"}
            loc._current_lang = "en_US"
            result = tr("不存在的翻译")
...
class TestTrMissingPrompts:
    def test_ja_jp_missing(self):
        ...
        original_lang = loc._current_lang
        try:
            # 非空翻译（模拟已加载），key 缺失 → ja_JP Missing
            loc._translations = {"其他key": "other"}
    ...
    def test_ko_kr_missing(self):
        ...
        original_lang = loc._current_lang
        try:
            # 非空翻译（模拟已加载），key 缺失 → ko_KR Missing
            loc._translations = {"其他key": "other"}
